...
# Canal de reportes de blacklist
BLACKLIST_CHANNEL_ID = -1002647649830


# Función para resolver conflictos de bot (simplificada)
def resolver_conflicto_bot():
    """Limpia updates pendientes"""
    try:
        print("🔧 Limpiando updates pendientes...")
        return True
...
async def enviar_reporte_blacklist(context, usuario_data):
    """Enviar reporte de nuevo usuario baneado al canal"""
    try:
        # Crear mensaje de reporte para el canal
        mensaje_canal = f"🚫 NUEVO USUARIO BANEADO\n\n"
        mensaje_canal += f"👤 Usuario: {usuario_data.get('username', 'N/A')}\n"
        mensaje_canal += f"🆔 ID: {usuario_data.get('user_id', 'N/A')}\n"
        mensaje_canal += f"⚠️ Motivo: {usuario_data.get('motivo', 'N/A')}\n"
        mensaje_canal += f"📅 Fecha: {usuario_data.get('fecha_agregado', 'N/A')}\n\n"
        
        # Mostrar información de tarjetas si existen
        tarjetas = usuario_data.get('tarjetas', [])
        if tarjetas and tarjetas != ['N/A']:
            mensaje_canal += f"💳 Tarjetas comprometidas ({len(tarjetas)}):\n"
            for i, tarjeta in enumerate(tarjetas, 1):
                tarjeta_str = str(tarjeta).strip()
                if tarjeta_str and tarjeta_str != 'N/A':
                    # Formatear tarjeta si es un número completo
                    if len(tarjeta_str) >= 16 and tarjeta_str.isdigit():
                        tarjeta_formatted = ' '.join([tarjeta_str[j:j+4] for j in range(0, len(tarjeta_str), 4)])
                        mensaje_canal += f"   {i}. {tarjeta_formatted}\n"
                    else:
                        mensaje_canal += f"   {i}. {tarjeta_str}\n"
        
        # Mostrar teléfonos si existen
        telefono = usuario_data.get('telefono', 'N/A')
        if telefono and telefono != 'N/A':
            telefonos = telefono.split('\n')
            if len(telefonos) == 1:
                mensaje_canal += f"\n📱 Teléfono: {telefonos[0].strip()}\n"
                ...
                for i, tel in enumerate(telefonos, 1):
                    tel_clean = tel.strip()
                    if tel_clean:
                        mensaje_canal += f"   {i}. {tel_clean}\n"
        
        # Información adicional
        info_adicional = usuario_data.get('info_adicional', 'N/A')
        if info_adicional and info_adicional != 'N/A':
            mensaje_canal += f"\n📊 Información adicional:\n"
            mensaje_canal += f"• {info_adicional}\n"
        
        mensaje_canal += f"\n🔴 EVITAR INTERACCIONES CON ESTE USUARIO\n"
        mensaje_canal += f"⚠️ REPORTADO COMO PROBLEMÁTICO\n"
        mensaje_canal += f"📞 Canal oficial: @lista_negra_reportes"
        
        # Enviar al canal sin formato Markdown
        await context.bot.send_message(
            chat_id=BLACKLIST_CHANNEL_ID,
            text=mensaje_canal
        )
        print(f"✅ Reporte enviado al canal para usuario {usuario_data.get('username', 'N/A')}")
        return True
        
    except Exception as e:
        print(f"❌ Error enviando reporte al canal: {e}")
        return False
...
def obtener_teclado_principal(user_id, chat_type="private"):
    ...
    if es_admin:
        ...
        # Teclado específico para usuarios regulares (sin opción de blacklist)
        keyboard = [
            ["🔍 Búsqueda Inteligente", "💎 Verificación VIP"],
            ["📞 Contacto y Ayuda"]
        ]
        return ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=False)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        print(f"👤 Usuario: {username} (ID: {user_id}) en chat: {chat_type}")
        
        # Cargar datos básicos
        try:
            admins = cargar_admins()
            vips = cargar_vip_users()
            es_admin = user_id in admins
            es_vip = buscar_vip_por_id(user_id)
            print(f"✅ Datos cargados: {len(admins)} admins, {len(vips)} VIPs")
        except Exception as e:
            print(f"❌ Error cargando datos: {e}")
            await update.message.reply_text("❌ Error interno. Contacta @frankosmel")
            return

        # Mensaje básico
        mensaje = "🤖 **Bot VIP - Sistema de Verificación**\n\n"
        mensaje += f"👋 Hola @{username}\n\n"

        if es_admin:
            mensaje += "👑 **ADMINISTRADOR** - Acceso completo\n"
        elif es_vip:
            mensaje += "💎 **USUARIO VIP** - Verificado\n"
        else:
            mensaje += "👤 **Usuario Regular**\n"

        # Funciones según tipo de chat
        if chat_type in ["group", "supergroup"]:
            mensaje += "\n🔍 **Comandos en grupos:**\n"
            mensaje += "• `/vip @usuario` - Verificar VIP\n"
            mensaje += "• `/checkblacklist @usuario` - Verificar blacklist\n\n"
            mensaje += "💡 Escríbeme en privado para más funciones"
            keyboard = ReplyKeyboardRemove()
        else:
            # Chat privado
            if es_admin:
                mensaje += "\n🔧 **Panel Administrativo:**\n"
                mensaje += "• Gestión completa de usuarios VIP\n"
                mensaje += "• Control de blacklist y admins\n"
                mensaje += "• Búsquedas avanzadas\n\n"
            else:
                mensaje += "\n🔍 **Funciones disponibles:**\n"
                mensaje += "• Búsqueda inteligente de usuarios\n"
                mensaje += "• Verificación VIP\n"
                mensaje += "• Verificación de blacklist\n\n"
            
            mensaje += "📋 **Comandos:**\n"
            mensaje += "• `/vip @usuario` - Verificar VIP\n"
            mensaje += "• `/checkblacklist @usuario` - Verificar blacklist\n"
            if es_admin:
                mensaje += "• `/addvip` - Agregar VIP (manual)\n"
                mensaje += "• `/addadmin` - Agregar admin\n"
            mensaje += "\n📞 **Soporte:** @frankosmel"
        ...
        print("✅ /start completado exitosamente")
        
    except Exception as e:
        print(f"❌ ERROR en /start: {str(e)}")
        try:
            await update.message.reply_text("❌ Error del bot. Contacta @frankosmel")
        except:
...
async def manejar_mensaje_texto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # BOTONES PARA USUARIOS REGULARES
    elif texto == "🔍 Búsqueda Inteligente":
        # Función de búsqueda para usuarios normales
        keyboard = [
            [InlineKeyboardButton("🆔 Buscar por ID", callback_data="user_search_by_id")],
            [InlineKeyboardButton("👤 Buscar por @Usuario", callback_data="user_search_by_username")],
            [InlineKeyboardButton("🧠 Búsqueda Inteligente", callback_data="user_search_smart")]
        ]

        mensaje = "🔍 **Búsqueda Inteligente - Usuario Regular**\n\n"
        mensaje += "✅ **Funciones de verificación disponibles:**\n\n"
        mensaje += "🎯 **Opciones de búsqueda:**\n"
        ...
        mensaje += f"📊 **Sistema de verificación activo:**\n"
        mensaje += f"• 💎 Base de usuarios verificados\n"
        mensaje += f"• 🚫 Sistema de seguridad anti-estafas\n\n"
        mensaje += f"✅ **Función:** Verificar confiabilidad y seguridad de usuarios\n"
        mensaje += f"💡 **Útil para:** Verificar antes de intercambios o transacciones\n"
        mensaje += f"⚠️ **Nota:** Información básica de seguridad disponible\n\n"
        mensaje += f"🔽 **Selecciona el tipo de búsqueda:**"

        await update.message.reply_text(mensaje, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')

    elif texto == "💎 Verificación VIP":
        # Función de verificación VIP para usuarios normales
        keyboard = [
            [InlineKeyboardButton("👤 Verificar por @Usuario", callback_data="user_vip_check_username")],
            [InlineKeyboardButton("🆔 Verificar por ID", callback_data="user_vip_check_id")],
            [InlineKeyboardButton("📋 Lista VIP Pública", callback_data="user_vip_list")]
        ]

        mensaje = "💎 **Verificación VIP - Usuario Regular**\n\n"
        mensaje += "✅ **Verificación rápida de usuarios VIP:**\n\n"
        mensaje += "🎯 **Opciones disponibles:**\n"
        mensaje += "• 👤 **Por @Usuario:** Verificar si un username es VIP\n"
        mensaje += "• 🆔 **Por ID:** Verificar si un ID es VIP\n"
        mensaje += "• 📋 **Lista Pública:** Ver usuarios VIP verificados\n\n"
        mensaje += f"💎 **Sistema de verificación activo**\n\n"
        mensaje += "🛡️ **Función:** Confirmar estatus de confianza\n"
        mensaje += "✅ **Útil para:** Verificar antes de transacciones\n"
        mensaje += "📞 **Para ser VIP contacta:** @frankosmel\n\n"
        mensaje += "🔽 **Selecciona una opción:**"

        await update.message.reply_text(mensaje, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')

    elif texto == "📞 Contacto y Ayuda":
        # Función de contacto y ayuda para usuarios normales
        mensaje = "📞 **Contacto y Ayuda - Usuario Regular**\n\n"
        mensaje += "👋 **¡Hola! Aquí tienes toda la información de ayuda:**\n\n"
        
        mensaje += "🆘 **Soporte Principal:**\n"
        mensaje += "• 👑 **@frankosmel** - Administrador principal\n"
        mensaje += "• 📞 Soporte técnico y consultas\n"
        mensaje += "• 💎 Solicitar estatus VIP\n\n"
        
        mensaje += "📋 **Comandos disponibles para ti:**\n"
        mensaje += "• `/start` - Reiniciar el bot\n"
        mensaje += "• `/vip @usuario` - Verificar si un usuario es VIP\n"
        mensaje += "• `/checkblacklist @usuario` - Verificar lista negra\n\n"
        
        mensaje += "🔍 **Funciones del teclado:**\n"
        mensaje += "• 🔍 **Búsqueda Inteligente:** Verificar usuarios en todas las bases\n"
        mensaje += "• 💎 **Verificación VIP:** Confirmar estatus VIP\n"
        mensaje += "• 🚫 **Verificar Blacklist:** Comprobar usuarios baneados\n\n"
        
        mensaje += "💡 **¿Para qué usar el bot?**\n"
        mensaje += "• ✅ Verificar confiabilidad antes de intercambios\n"
        mensaje += "• 🛡️ Confirmar identidad de usuarios VIP\n"
        mensaje += "• ⚠️ Evitar usuarios problemáticos\n"
        mensaje += "• 🔍 Buscar información de seguridad\n\n"
        
        mensaje += "❓ **¿Cómo obtener estatus VIP?**\n"
        mensaje += "• Contacta a @frankosmel\n"
        mensaje += "• Proporciona tu información KYC\n"
        mensaje += "• Espera verificación administrativa\n\n"
        ...
        keyboard = [
            [InlineKeyboardButton("💬 Contactar @frankosmel", url="https://t.me/frankosmel")],
            [InlineKeyboardButton("📖 Guía de Uso", callback_data="user_guide")],
            [InlineKeyboardButton("❓ Preguntas Frecuentes", callback_data="user_faq")]
        ]
...
async def test_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"🧪 TEST recibido de {username} (ID: {user_id})")
    ...
    try:
        ...
        mensaje = "🧪 **TEST DEL BOT**\n\n"
        mensaje += f"✅ Bot funcionando correctamente\n"
        mensaje += f"👤 Usuario: @{username} (ID: {user_id})\n"
        mensaje += f"🔐 Admin: {'✅' if user_id in admins else '❌'}\n"
        mensaje += f"💎 VIP: {'✅' if buscar_vip_por_id(user_id) else '❌'}\n\n"
        mensaje += f"📊 **Base de datos:**\n"
        mensaje += f"• Admins: {len(admins)}\n"
        mensaje += f"• VIPs: {len(vips)}\n"
        mensaje += f"• Blacklist: {len(blacklist)}\n\n"
...
# Función para manejar submenús de administración
async def admin_submenu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Solo verificar permisos para funciones específicamente de administrador
    admin_functions = [
        "admin_", "vip_add", "vip_remove", "vip_list", "blacklist_add", "blacklist_remove", 
        "blacklist_list", "mass_message", "config_", "confirm_", "cancel_admin", "cancel_vip", 
        "cancel_blacklist", "start_", "admin_panel", "vip_panel", "blacklist_menu"
    ]
    
    is_admin_function = any(data.startswith(prefix) for prefix in admin_functions)
    
    if is_admin_function and user_id not in cargar_admins():
    ...
    # Handlers para verificación VIP rápida
        ...
        # Verificar si es administrador para mostrar estadísticas completas o básicas
        if user_id in cargar_admins():
            ...
            mensaje += f"✅ **Sistema de verificación activo**\n\n"
            mensaje += f"🛡️ **Funciones disponibles:**\n"
            mensaje += f"• Sistema de usuarios verificados\n"
            mensaje += f"• Protección anti-estafas\n"
            mensaje += f"• Verificación de confiabilidad\n\n"
            mensaje += f"📅 **Sistema actualizado y funcionando**\n"
            mensaje += f"💡 **Usa las búsquedas para verificar usuarios antes de intercambios**\n\n"
            mensaje += f"🕐 **Consultado:** {datetime.now().strftime('%d/%m/%Y %H:%M')}"

        keyboard = [[InlineKeyboardButton("🔙 Volver", callback_data="main_search_menu")]]
        await query.edit_message_text(mensaje, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')

    # HANDLERS PARA USUARIOS REGULARES
    elif data.startswith("user_search_"):
        search_type = data.replace("user_search_by_", "").replace("user_search_", "")
        
        if search_type == "id":
            mensaje = "🆔 **Búsqueda por ID - Usuario Regular**\n\n"
            mensaje += "📝 **Instrucciones:**\n"
            mensaje += "• Escribe el ID de Telegram a verificar\n"
            mensaje += "• Ejemplo: 1383931339\n"
            mensaje += "• Solo números, sin espacios ni símbolos\n\n"
            mensaje += "🔍 **Se verificará en el sistema de seguridad:**\n"
            mensaje += "• 💎 Base de usuarios verificados\n"
            mensaje += "• 🚫 Sistema anti-estafas\n\n"
            mensaje += "✅ **Función:** Verificar confiabilidad y seguridad\n"
            mensaje += "💡 **Útil para:** Verificar antes de intercambios\n\n"
            mensaje += "📝 **Escribe el ID a verificar:**"

            user_states[user_id] = {'global_search': True, 'search_type': 'id', 'user_mode': True}
            keyboard = [[InlineKeyboardButton("❌ Cancelar", callback_data="cancel_user_search")]]
            await query.edit_message_text(mensaje, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')

        elif search_type == "username":
            mensaje = "👤 **Búsqueda por @Usuario - Usuario Regular**\n\n"
            mensaje += "📝 **Instrucciones:**\n"
            mensaje += "• Escribe el username a verificar\n"
            mensaje += "• Ejemplo: @frankosmel o frankosmel\n"
            mensaje += "• Con o sin el símbolo @\n\n"
            ...
            mensaje += "• 💎 Base de usuarios verificados\n"
            mensaje += "• 🚫 Sistema anti-estafas\n\n"
            mensaje += "✅ **Función:** Verificar confiabilidad\n"
            mensaje += "💡 **Útil para:** Verificar antes de transacciones\n\n"
            mensaje += "📝 **Escribe el @usuario a verificar:**"

            user_states[user_id] = {'global_search': True, 'search_type': 'username', 'user_mode': True}
            keyboard = [[InlineKeyboardButton("❌ Cancelar", callback_data="cancel_user_search")]]
            await query.edit_message_text(mensaje, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
            ...
            mensaje = "🧠 **Búsqueda Inteligente - Usuario Regular**\n\n"
            mensaje += "🤖 **Detección automática del tipo de dato:**\n"
            mensaje += "• El sistema detectará automáticamente qué escribes\n"
            mensaje += "• Verificación en el sistema de seguridad\n"
            mensaje += "• Resultados con información de seguridad\n\n"
            mensaje += "📝 **Ejemplos de verificación:**\n"
            mensaje += "• @usuario o usuario - Verificar username\n"
            mensaje += "• 1234567890 - Verificar ID de Telegram\n\n"
            ...
            user_states[user_id] = {'global_search': True, 'search_type': 'smart', 'user_mode': True}
            keyboard = [[InlineKeyboardButton("❌ Cancelar", callback_data="cancel_user_search")]]
            await query.edit_message_text(mensaje, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')

    elif data.startswith("user_vip_check_"):
        check_type = data.replace("user_vip_check_", "")
        ...
        if check_type == "username":
            ...
            user_states[user_id] = {'quick_vip_verify': True, 'verify_type': 'username'}
            keyboard = [[InlineKeyboardButton("❌ Cancelar", callback_data="cancel_user_vip_check")]]
            await query.edit_message_text(mensaje, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
        ...
        keyboard = [[InlineKeyboardButton("🔙 Volver", callback_data="back_to_user_menu")]]
        await query.edit_message_text(mensaje, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')

    

    elif data == "user_guide":
        mensaje = "📖 **Guía de Uso del Bot - Usuario Regular**\n\n"
        mensaje += "👋 **¡Bienvenido! Aquí aprenderás a usar todas las funciones:**\n\n"
        
        mensaje += "🔍 **1. Búsqueda Inteligente:**\n"
        mensaje += "• Usar cuando quieras verificar cualquier usuario\n"
        mensaje += "• Escribe @usuario, ID o teléfono\n"
        mensaje += "• El bot te dice si es confiable o no\n\n"
        
        mensaje += "💎 **2. Verificación VIP:**\n"
        mensaje += "• Usar para confirmar usuarios VIP\n"
        mensaje += "• Usuarios VIP son 100% confiables\n"
        mensaje += "• Tienen respaldo administrativo\n\n"
        
        mensaje += "🚫 **3. Verificar Blacklist:**\n"
        mensaje += "• Usar para evitar usuarios problemáticos\n"
        mensaje += "• Te alerta de usuarios reportados\n"
        mensaje += "• Protege tu seguridad\n\n"
        
        mensaje += "📞 **4. Comandos útiles:**\n"
        mensaje += "• `/vip @usuario` - Verificar VIP rápido\n"
        mensaje += "• `/checkblacklist @usuario` - Verificar baneados\n"
        mensaje += "• `/start` - Reiniciar el bot\n\n"
        
        mensaje += "💡 **Consejos de uso:**\n"
        mensaje += "• Siempre verifica antes de intercambios\n"
        mensaje += "• Si alguien está en blacklist, no interactúes\n"
        mensaje += "• Usuarios VIP son 100% seguros\n"
        mensaje += "• Contacta @frankosmel para dudas"

        keyboard = [[InlineKeyboardButton("🔙 Volver", callback_data="back_to_user_menu")]]
        await query.edit_message_text(mensaje, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')

    elif data == "user_faq":
        mensaje = "❓ **Preguntas Frecuentes - Usuario Regular**\n\n"
        
        mensaje += "**Q: ¿Qué es un usuario VIP?**\n"
        mensaje += "A: Usuario verificado por administradores con KYC completo\n\n"
        
        mensaje += "**Q: ¿Cómo puedo ser VIP?**\n"
        mensaje += "A: Contacta a @frankosmel con tu información\n\n"
        
        mensaje += "**Q: ¿Qué es la blacklist?**\n"
        mensaje += "A: Lista de usuarios reportados como problemáticos\n\n"
        
        mensaje += "**Q: ¿Por qué verificar usuarios?**\n"
        mensaje += "A: Para evitar estafas y mantener transacciones seguras\n\n"
        
        mensaje += "**Q: ¿Es gratis usar el bot?**\n"
        mensaje += "A: Sí, todas las verificaciones son completamente gratis\n\n"
        
        mensaje += "**Q: ¿Qué hago si encuentro un estafador?**\n"
        mensaje += "A: Reporta inmediatamente a @frankosmel\n\n"
        
        mensaje += "**Q: ¿Los datos son privados?**\n"
        mensaje += "A: Sí, solo información básica es visible públicamente\n\n"
        
        mensaje += "**Q: ¿Funciona 24/7?**\n"
        mensaje += "A: Sí, el bot está disponible las 24 horas"

        keyboard = [[InlineKeyboardButton("🔙 Volver", callback_data="back_to_user_menu")]]
        await query.edit_message_text(mensaje, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')

    # Handlers de cancelación para usuarios regulares
    elif data == "cancel_user_search":
        if user_id in user_states:
            del user_states[user_id]
        await query.edit_message_text("❌ **Búsqueda cancelada**", parse_mode='Markdown')

    elif data == "cancel_user_vip_check":
        if user_id in user_states:
            del user_states[user_id]
        await query.edit_message_text("❌ **Verificación VIP cancelada**", parse_mode='Markdown')
        ...
        # Volver al menú principal con teclado para usuarios regulares
        keyboard = obtener_teclado_principal(user_id, "private")
        mensaje = "🔙 **Menú Principal**\n\n"
        mensaje += "👤 Usuario: Menú principal restaurado\n"
        mensaje += "🔽 Utiliza los botones para las funciones disponibles:"
...
async def handle_blacklist_creation_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "confirm_blacklist_creation":
        ...
        # Enviar reporte al canal de blacklist
        reporte_enviado = await enviar_reporte_blacklist(context, nuevo_baneado)
        ...
        if reporte_enviado:
            mensaje += "📢 **Reporte enviado al canal de blacklist automáticamente**"
        else:
            mensaje += "⚠️ **Error enviando reporte al canal (usuario baneado correctamente)**"
    ...
    if data.startswith("aceptar_") or data.startswith("rechazar_"):
        ...
        if usuario_que_presiona != vip_user_id:
            ...
            try:
                ...
                mensaje_advertencia += f"⏰ **Este mensaje se eliminará automáticamente en 40 segundos.**"
                ...
                # Programar eliminación automática del mensaje después de 40 segundos
                async def delete_warning_message():
                    try:
                        await asyncio.sleep(40)
                        await sent_message.delete()
                        print(f"✅ Mensaje de advertencia eliminado automáticamente para usuario {username_quien_presiona}")
                    except Exception as e:
                        print(f"No se pudo eliminar el mensaje de advertencia: {e}")
...
app.add_handler(CommandHandler("test", test_bot))
...
async def handle_universal_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if search_results['blacklist_multiple']:
        ...
        for i, blacklist_user in enumerate(search_results['blacklist_multiple'], 1):
            ...
            # Para usuarios normales, mostrar solo información básica de seguridad
            if not es_admin:
                mensaje += f"   📅 Reportado como problemático\n\n"
            else:
        ...
        for i, vip_user in enumerate(search_results['vip_multiple'], 1):
            ...
            if es_admin:
                ...
                if search_results['search_type'] == 'phone':
                    ...
                    mensaje += f"   💳 CUP: {vip_user.get('tarjeta_cup', 'N/A')}\n"
                mensaje += f"   📅 Registrado: {vip_user.get('fecha_agregado', 'N/A')}\n"
                mensaje += f"   👑 Verificado por: {vip_user.get('agregado_por', 'N/A')}\n\n"
            else:
                mensaje += f"   📅 Usuario verificado oficialmente\n\n"
...
# Handler para usuarios normales (funciones públicas)
app.add_handler(CallbackQueryHandler(admin_submenu_handler, pattern="^(user_|global_search|universal_search|quick_vip_verify|public_vip_list|cancel_user|cancel_global|back_to_|user_guide|user_faq)"))
# Handler principal para administradores
app.add_handler(CallbackQueryHandler(admin_submenu_handler))

print("=" * 50)
print("✅ BOT VIP INICIADO CORRECTAMENTE")
print("=" * 50)
print(f"🔑 Token configurado: {app.bot.token[:10]}...{app.bot.token[-5:]}")
print(f"📱 Bot username: Verificando...")
print(f"🌐 Puerto Flask: 80")
print(f"📊 Estados de usuario activos: {len(user_states)}")
print("=" * 50)
...
# Ejecutar bot con manejo simplificado
# --- Entrada original removida por refactor ---
from config import BOT_TOKEN
...
"""
Este bloque ha sido añadido automáticamente para:

1. Iniciar el servidor Flask (health‑check) en un hilo separado.
2. Arrancar el bot en modo polling con reconexión automática.
3. Manejar cierre limpio ante SIGINT/SIGTERM.
"""
...
def iniciar_flask():
    ...
    logging.info("Flask iniciado en hilo separado.")
...
async def main():
    ...
    logging.basicConfig(
        ...
        level=logging.INFO,
        handlers=[
            logging.FileHandler("bot_vip.log"),
    ...
    if not token:
        logging.error("La variable de entorno BOT_TOKEN no está definida.")
        return

    # Construir aplicación de Telegram
    bot_app = Application.builder().token(token).build()

    # ----- Registro de handlers existentes -----
    # Se asume que tus funciones/handlers (start, vip_command, etc.)
    # están definidos en el código original mantenido arriba.
    try:
        bot_app.add_handler(CommandHandler("start", start))
        bot_app.add_handler(CommandHandler("vip", vip_command))
        bot_app.add_handler(CommandHandler("addvip", add_vip))
        bot_app.add_handler(CommandHandler("delvip", eliminar_vip))
        bot_app.add_handler(CommandHandler("addadmin", add_admin))
        bot_app.add_handler(CommandHandler("deladmin", eliminar_admin))
        bot_app.add_handler(CallbackQueryHandler(admin_submenu_handler))
        bot_app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, manejar_mensaje_texto))
    except NameError as err:
        logging.warning(f"Algunos handlers no se pudieron registrar automáticamente: {err}")

    # Arranque del polling
    await bot_app.initialize()
    await bot_app.start()
    logging.info("Bot iniciado y recibiendo actualizaciones...")
