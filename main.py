...
if __name__ == "__main__":
    ...
    try:
        print("🚀 Iniciando Bot VIP...")
        print(f"👑 Administradores: {len(cargar_admins())}")
        print(f"💎 Usuarios VIP: {len(cargar_vip_users())}")
        print(f"🚫 Blacklist: {len(cargar_blacklist())}")
        ...
        print("   1. Detén el Repl (Stop)")
        print("   2. Espera 10 segundos")
        print("   3. Ejecuta de nuevo (Run)")
...
# ================== NUEVO PUNTO DE ENTRADA (generado) ==================
import asyncio
import logging
import os
import signal
...
async def main():
    ...
    if not token:
        logging.error("BOT_TOKEN no definido")
        return
    ...
    await app.stop()
    await app.shutdown()
    logging.info("Bot detenido limpiamente")
